    ...
    return `<div class="plugin-card" onclick="showPluginDetail('${p.plugin_id}')">
      ...
      <div class="plugin-card-footer">
        ...
        <span class="plugin-card-quick">
          <button class="qbtn" onclick="event.stopPropagation();quickFavorite('${p.plugin_id}')" title="Add to Favorites" aria-label="Adicionar aos favoritos">⭐</button>
  ...
  container.innerHTML = customLists.map(list => {
    ...
    return `<div class="list-card">
      ...
      <div class="list-card-items">${pluginDetails.length ? pluginDetails.map(p =>
        `<div class="list-plugin-item"><span>${p.name}</span><button class="btn btn-sm btn-ghost" onclick="removeFromList('${list.id}','${p.plugin_id}')" aria-label="Remover da lista" title="Remover">✕</button></div>`
  ...
  c.innerHTML = workspaces.map(ws => {
    ...
    return `<div class="ws-card">
      ...
      <div class="ws-plugin-grid" id="ws-plugins-${ws.id}">
        ${ws.plugins.map(wp => {
          ...
          return `<div class="ws-plugin-item ${wp.enabled?'':'disabled'}">
            ...
            <button class="btn btn-sm btn-ghost" onclick="removeWsPlugin('${ws.id}','${wp.plugin_id}')" title="Remover" aria-label="Remover do workspace">✕</button>
          </div>`;
        }).join('')}
      ...
      <div class="ws-card-footer">
        ...
        <button class="btn btn-sm btn-danger" onclick="deleteWorkspace('${ws.id}')" aria-label="Excluir workspace" title="Excluir">🗑</button>
      </div>
    </div>`;
  }).join('');
}
