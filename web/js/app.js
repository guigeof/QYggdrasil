    ...
    return `<div class="plugin-card" onclick="showPluginDetail('${p.plugin_id}')">
      ...
      <div class="plugin-card-footer">
        ...
        <span class="plugin-card-quick">
          <button class="qbtn" aria-label="Adicionar aos favoritos" onclick="event.stopPropagation();quickFavorite('${p.plugin_id}')" title="Add to Favorites">⭐</button>
          <button class="qbtn" aria-label="Adicionar ao workspace" onclick="event.stopPropagation();quickAddWs('${p.plugin_id}')" title="Add to Workspace">⚡</button>
        </span>
      </div>
    </div>`;
  }).join('');
  ...
  container.innerHTML = customLists.map(list => {
    ...
    return `<div class="list-card">
      ...
      <div class="list-card-items">${pluginDetails.length ? pluginDetails.map(p =>
        `<div class="list-plugin-item"><span>${p.name}</span><button class="btn btn-sm btn-ghost" aria-label="Remover da lista" onclick="removeFromList('${list.id}','${p.plugin_id}')">✕</button></div>`
      ...
      <div class="ws-plugin-grid" id="ws-plugins-${ws.id}">
        ${ws.plugins.map(wp => {
          ...
          return `<div class="ws-plugin-item ${wp.enabled?'':'disabled'}">
            ...
            <button class="btn btn-sm btn-ghost" aria-label="Remover do workspace" onclick="removeWsPlugin('${ws.id}','${wp.plugin_id}')" title="Remover">✕</button>
          </div>`;
        }).join('')}
