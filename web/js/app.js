...
function renderPagination() {
  ...
  if (currentPage < totalPages) html += `<button class="page-btn" onclick="goPage(${currentPage+1})" aria-label="Próxima página">›</button>`;
  ...
  c.innerHTML = workspaces.map(ws => {
    ...
    return `<div class="ws-card">
      ...
      <div class="ws-plugin-grid" id="ws-plugins-${ws.id}">
        ${ws.plugins.map(wp => {
          ...
          return `<div class="ws-plugin-item ${wp.enabled?'':'disabled'}">
            ...
            <button class="btn btn-sm btn-ghost" onclick="removeWsPlugin('${ws.id}','${wp.plugin_id}')" title="Remover do workspace" aria-label="Remover do workspace">✕</button>
          </div>`;
        }).join('')}
      ...
      <div class="ws-card-footer">
        ...
        <button class="btn btn-sm btn-danger" onclick="deleteWorkspace('${ws.id}')" title="Excluir workspace" aria-label="Excluir workspace">🗑</button>
      </div>
    </div>`;
  }).join('');
}
