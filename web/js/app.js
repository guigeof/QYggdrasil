    ...
    return `<div class="plugin-card" onclick="showPluginDetail('${p.plugin_id}')">
      ...
      <div class="plugin-card-footer">
        ...
        <span class="plugin-card-quick">
          <button class="qbtn" onclick="event.stopPropagation();quickFavorite('${p.plugin_id}')" title="Add to Favorites" aria-label="Favoritar">⭐</button>
          <button class="qbtn" onclick="event.stopPropagation();quickAddWs('${p.plugin_id}')" title="Add to Workspace" aria-label="Adicionar ao Workspace">⚡</button>
        </span>
      </div>
    </div>`;
  }).join('');
...
function renderPagination() {
  ...
  let html = '';
  const range = 3;
  if (currentPage > 1) html += `<button class="page-btn" onclick="goPage(${currentPage-1})" aria-label="Página anterior">‹</button>`;
  for (let i = 1; i <= totalPages; i++) {
    if (i === 1 || i === totalPages || (i >= currentPage-range && i <= currentPage+range)) {
      html += `<button class="page-btn ${i===currentPage?'active':''}" onclick="goPage(${i})">${i}</button>`;
    } else if (i === currentPage-range-1 || i === currentPage+range+1) {
      html += `<button class="page-btn" disabled aria-hidden="true">…</button>`;
    }
  }
  if (currentPage < totalPages) html += `<button class="page-btn" onclick="goPage(${currentPage+1})" aria-label="Página seguinte">›</button>`;
  ...
  container.innerHTML = customLists.map(list => {
    ...
    return `<div class="list-card">
      ...
      <div class="list-card-items">${pluginDetails.length ? pluginDetails.map(p =>
        `<div class="list-plugin-item"><span>${p.name}</span><button class="btn btn-sm btn-ghost" onclick="removeFromList('${list.id}','${p.plugin_id}')" aria-label="Remover plugin da lista">✕</button></div>`
  ...
  c.innerHTML = workspaces.map(ws => {
    ...
    return `<div class="ws-card">
      ...
      <div class="ws-plugin-grid" id="ws-plugins-${ws.id}">
        ${ws.plugins.map(wp => {
          ...
          return `<div class="ws-plugin-item ${wp.enabled?'':'disabled'}">
            ...
            <button class="btn btn-sm btn-ghost" onclick="removeWsPlugin('${ws.id}','${wp.plugin_id}')" title="Remover" aria-label="Remover plugin do workspace">✕</button>
          </div>`;
        }).join('')}
      ...
      <div class="ws-card-footer">
        ...
        <button class="btn btn-sm btn-danger" onclick="deleteWorkspace('${ws.id}')" aria-label="Excluir workspace">🗑</button>
      </div>
    </div>`;
  }).join('');
}
