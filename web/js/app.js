  ...
  container.innerHTML = customLists.map(list => {
    ...
    return `<div class="list-card">
      ...
      <div class="list-card-items">${pluginDetails.length ? pluginDetails.map(p =>
        `<div class="list-plugin-item"><span>${p.name}</span><button class="btn btn-sm btn-ghost" aria-label="Remover" onclick="removeFromList('${list.id}','${p.plugin_id}')">✕</button></div>`
      ...
      <div class="ws-plugin-grid" id="ws-plugins-${ws.id}">
        ${ws.plugins.map(wp => {
          ...
          return `<div class="ws-plugin-item ${wp.enabled?'':'disabled'}">
            ...
            <button class="btn btn-sm btn-ghost" aria-label="Remover" onclick="removeWsPlugin('${ws.id}','${wp.plugin_id}')" title="Remover">✕</button>
          </div>`;
        }).join('')}
